...
const EditListingForm = ({ listing }) => {
  // State to track post button enable
  ...
  const [propertyInfo, setPropertyInfo] = useState({
    ...
    stayPrice: listing?.stayPrice || 0,
  });

  // State for address info
  const [addressInfo, setAddressInfo] = useState({
    doorNumber: listing?.address?.doorNumber || "",
    streetOrLocality: listing?.address?.streetOrLocality || "",
  ...
  const [propertyDetails, setPropertyDetails] = useState({
    ...
    kitchen: listing?.propertyDetails?.kitchen || "",
    furnishType: listing?.propertyDetails?.furnishType || "",
    parking: listing?.propertyDetails?.parking || "",
    floorArea: listing?.propertyDetails?.floorArea || 0,
  });
  ...
  const [coverPhoto, setCoverPhoto] = useState(listing?.coverPhoto || null);

  // State for property media images
  const [propertyMedia, setPropertyMedia] = useState(listing?.propertyMedia || []);
  ...
  const handleMediaImageUpload = async (event) => {
    ...
    if (file) {
      ...
      if (imageUrl) {
        setPropertyMedia([...propertyMedia, imageUrl]);
      }
    }
  };
  ...
  const handleMediaImageDelete = (imageUrl) => {
    setPropertyMedia(propertyMedia.filter((url) => url !== imageUrl));
  };
  ...
  const updateListing = async () => {
    try {
      ...
      const response = await axios.put(
        `/api/user/update-listing?listingId=${listing._id}`,
        {
          propertyDetails,
          addressInfo,
          propertyInfo,
  ...
  // Enable post button if all required fields are filled
  useEffect(() => {
    if (
  ...
  }, [addressInfo, coverPhoto, propertyDetails, propertyInfo, propertyMedia]);
  ...
  return (
    <div className="space-y-6 p-8 max-w-4xl mx-auto">
      ...
      >
        ...
        <AccordionItem value="item-1">
          ...
          <AccordionContent className="bg-white p-6 space-y-6 shadow-md rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {/* Conditionally render Stay Price based on the listingType */}
                ...
                <div className="space-y-2">
                  {[
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={amenities.accessibility[amenity]}
                        onChange={() =>
                          handleAmenitiesChange("accessibility", amenity)
                        }
                      />
                      <span>{amenity.replace(/([A-Z])/g, " $1")}</span>
                    </label>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={amenities.security[amenity]}
                        onChange={() =>
                          handleAmenitiesChange("security", amenity)
                        }
                      />
                      <span>{amenity.replace(/([A-Z])/g, " $1")}</span>
                    </label>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={amenities.pet[amenity]}
                        onChange={() =>
                          handleAmenitiesChange("pet", amenity)
                        }
                      />
                      <span>{amenity.replace(/([A-Z])/g, " $1")}</span>
                    </label>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={amenities.additional[amenity]}
                        onChange={() =>
                          handleAmenitiesChange("additional", amenity)
                        }
                      />
                      <span>{amenity.replace(/([A-Z])/g, " $1")}</span>
                    </label>
        ...
        {/* Property Cover Photo Accordion */}
