...
const ListingMedia = ({ coverPhoto, propertyMedia = [] }) => {
  ...
  return (
    <div className="relative container mx-auto flex flex-col md:flex-row gap-4 items-center md:items-start justify-center p-4">
      ...
      <div className="w-full md:w-2/3 rounded-lg overflow-hidden shadow-xl relative">
        <motion.img
          src={coverPhoto}
          alt="Cover"
          className="w-full h-auto md:h-[320px] object-cover rounded-lg transition-transform duration-300 hover:scale-105"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <Dialog open={open} onOpenChange={setOpen} modal={true}>
          <DialogTrigger asChild>
            <Button
              type="button"
              className="absolute bottom-4 right-4 flex items-center gap-2 bg-gray-900 text-white hover:bg-gray-800 shadow-lg px-4 py-2 rounded-full transition-all"
            >
              <Eye size={20} />
              View All Photos
            </Button>
      ...
      <div className=" relative md:grid grid-cols-2 gap-2 p-1 hidden  md:w-1/3 max-h-[400px]">
        {propertyMedia.slice(0, 4).map((imgUrl, index) => (
          <motion.img
            key={index}
            src={imgUrl}
            alt={`media-${index}`}
            className="w-full object-cover rounded-lg shadow-md cursor-pointer transition-transform duration-300 hover:scale-105"
