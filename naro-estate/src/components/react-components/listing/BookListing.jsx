...
const BookListing = ({ listing }) => {
  // Combine pendingBookings and bookingsConfirmed into a single array of Date objects
  const [bookedDates, setBookedDates] = useState(
    [...listing.pendingBookings, ...listing.bookingsConfirmed].map((date) => new Date(date))
  );
  ...
  const ReserveListing = async () => {
    try {
      ...
      toast({
        ...
        description: data?.message || "Reserved Successfully. You will be redirected to the payments page.",
      });
      ...
      toast({
        ...
        description: error?.response.data?.message || "Couldn't reserve the listing!",
        variant: "destructive",
      });
  ...
  return (
    <Card className="max-w-md mx-auto p-6 border rounded-lg shadow-lg">
      <CardContent>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4 border p-3 rounded-lg bg-gray-50 shadow-md">
          <div>
            ...
            <DatePicker
              ...
              excludeDates={bookedDates} // Combined array of pending and confirmed bookings
              minDate={new Date()}
        ...
        <Button
          className="w-full bg-gradient-to-r from-pink-500 to-red-500 text-white py-2"
